...
public class SlimeStateClientHandler {
    ...
    public static void updateSlimeState(Player player, ItemStack stack, UUID uuid) {
        ...
        boolean inHand = uuid.equals(mainHand.get(ModComponentsCommon.SLIME_UUID)) ||
                         uuid.equals(offHand.get(ModComponentsCommon.SLIME_UUID));
        ...
        if (inHand) {
            ...
            // --- 採掘直後（タイマー中） ---
                    ...
                    !SlimeState.SLIME.equals(stack.get(ModComponentsCommon.SLIME_STATE)) &&
                    !SlimeState.SWORD.equals(stack.get(ModComponentsCommon.SLIME_STATE))) {
                // タイマー中は前のstateを維持
                state = player.totalExperience >= 1 ? stack.get(ModComponentsCommon.SLIME_STATE) : SlimeState.SLIME;
    ...
    public static void applyStateOnBlockUse(ItemStack stack, ToolRightClickHandler.ToolAction action) {
        ...
        slimeTimers.remove(uuid);
        SlimeState.apply(stack, newState);
    }
}