...
public class SlimeModeServerHandler {
    ...
    public static void handleUseMode(ServerPlayer player) {
        ...
        // モードを更新
        stack.set(ModComponentsCommon.SLIME_MODE, comp.withUseMode(newUseMode));
    ...
    public static void handleMiningMode(ServerPlayer player) {
        ...
        // エンチャントレベルコンポーネントを更新
        stack.set(ModComponentsCommon.MINING_ENCHANT_LEVEL, miningComp);
        ...
        // モードを更新
        stack.set(ModComponentsCommon.SLIME_MODE, comp.withMiningMode(newMiningMode));
    ...
    public static void updateMiningModeForInventory(UUID stackUuid, String mode, ServerPlayer player) {
        ...
        for (int i = 0; i < inventory.getContainerSize(); i++) {
            ...
            UUID uuidComp = stack.get(ModComponentsCommon.SLIME_UUID);
            if (uuidComp == null || !uuidComp.equals(stackUuid)) continue;

            SlimeModeComponent modeComp = stack.get(ModComponentsCommon.SLIME_MODE);
            if (modeComp == null) continue;

            // マイニングモードを更新
            SlimeModeComponent newComp = modeComp.withMiningMode(mode);
            stack.set(ModComponentsCommon.SLIME_MODE, newComp);
        }
    }
